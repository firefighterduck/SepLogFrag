...
proof (rule entailment_lift)
  define xs where xs: "xs = -(fv E\<^sub>1 \<union> fv E\<^sub>2 \<union> fv E\<^sub>3)"
  fix s h
  ...
  have "\<forall>x \<in> xs. (s(x:=\<lbrakk>E\<^sub>2\<rbrakk>s),h2_1)\<Turnstile>ls\<^sup>m(\<acute>x`,E\<^sub>3)"
  proof
    fix x
    assume "x \<in> xs"
    with xs have "x \<notin> fv E\<^sub>2 \<union> fv E\<^sub>3" by fast
    ...
    from ls_change_fst[OF tail_ext this] show "(s(x:=\<lbrakk>E\<^sub>2\<rbrakk>s),h2_1)\<Turnstile>ls\<^sup>m(\<acute>x`,E\<^sub>3)" .
  qed
  from sep_conj sep_conj2 have "h1\<bottom>h2_1" by auto
  from xs have "xs \<subseteq> -(fv E\<^sub>1 \<union> fv E\<^sub>3)" by blast
  have "h1++h2_1=h1++h2_1" by simp
  from ListSegment[OF hd \<open>xs \<subseteq> -(fv E\<^sub>1 \<union> fv E\<^sub>3)\<close> \<open>\<forall>x \<in> xs. (s(x:=\<lbrakk>E\<^sub>2\<rbrakk>s),h2_1)\<Turnstile>ls\<^sup>m(\<acute>x`,E\<^sub>3)\<close> \<open>h1\<bottom>h2_1\<close>
    this n \<open>\<lbrakk>E\<^sub>1\<rbrakk>s\<noteq>\<lbrakk>E\<^sub>3\<rbrakk>s\<close>] have "(s,h1++h2_1)\<Turnstile>ls\<^sup>n(E\<^sub>1,E\<^sub>3)" .
  {
